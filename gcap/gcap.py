...
class GCap():
    ...
    def _get_address(iface):
        if platform.system() == 'Linux':
            ...
            class union_ifreq_anon(ctypes.Union):
                _fields_ = [
                    ('ifr_hwaddr', struct_sockaddr),
                    ('raw', ctypes.c_short * 15),
                ]
            ...
            with contextlib.closing(
                ...
                ifr = struct_ifreq()
                ifr.ifr_name = iface[:16].encode('utf-8')
                _libc = ctypes.CDLL(ctypes.util.find_library('libc'))
                result = _libc.ioctl(
                    sock.fileno(),
                    SIOCGIFHWADDR,
                    ctypes.pointer(ifr),
            ...
            if result == 0:
                return ''.join(
                    '%02x:' % pair
                    for pair in ifr.ifr_hwaddr.sa_data[:6]
                )[:-1]
    ...
    def __init__(self, iface, timeout=1000):
        self._iface = iface
